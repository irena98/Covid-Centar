...
Ovaj rad predstavlja projekat iz predmeta Informacioni sistemi na master studijama Matematičkog fakulteta. Rad opisuje informacioni sistem kovid centra. U skladu sa trenutnom situacijom u našoj zemlji, smatramo da je izuzetno važno da ovakav sistem postoji i da omogućava efikasno i pouzdano funkcionisanje jednog kovid centra. \newline
\indent Dakle, ideja je da se sistem koristi prilikom rada jednog kovid centra. Komunikacija sa spoljnim zdravstvenim sistemom se vrši putem periodičnog slanja odgovarajućih podataka, a komunikacija se takođe dešava prilikom dobavljanja zaliha vakcina i testova. U okviru analize sistema su prepoznati osnovni procesi i učesnici u sistemu. \newline
\indent Za izradu dijagrama u okviru projekta koristili smo alate Visual Paradigm i Modelio.

\section{Analiza sistema}
Informacioni sistem Kovid centar svojim funkcionalnostima obezbeđuje sigurno i jednostavno sprovođenje procedura neophodnih za zaštitu od virusa COVID-19. \newline
\indent Pre svega, sistem koristi medicinsko osoblje prilikom prijavljivanja osoba za primanje vakcine, evidentiranja primljene doze, evidentiranja urađenog testa kao i praćenja poštovanja kućne izolacije. Pored toga, sistem omogućava da se podaci iz baze prikupe i proslede širem zdravstvenom sistemu u okviru kog radi kovid centar, kao i da se nabave nove zalihe vakcina i testova. Pri tom, smatramo da je medicinskom osoblju omogućeno korišćenje sistema prilikom zaposlenja.\newline
\indent Informacioni sistem se takođe koristi od strane osoba koje žele da koriste usluge vakcinisanja i testiranja. Sistem omogućava registraciju i online prijavu za vakcinu, kao i dobijanje kovid propusnice elektronskim putem. \newline
\indent Važan deo u borbi sa virusom je i kontrolisanje kovid propusnica, pa tako naš sistem omogućava i brzu proveru validnosti propusnice od strane bilo koga ko je u mogućnosti da očita QR kod sa kovid propusnice. \newline
\indent Još jedna upotreba informacionog sistema se ogleda prilikom komunikacije putem Kol centra, koji omogućava osobama da se u razgovoru sa stručnim licem informišu o virusu. \newline
\indent Naravno, da bi sistem ispravno funkcionisao, neophodna je administracija, pa je tako i ona obrađena u narednom poglavlju. \newline
...
\begin{figure}[H]
\centering
\includegraphics[scale=0.9]{Dijagram_konteksta}
...
\begin{figure}[H]
\centering
\includegraphics[scale=1.2]{DTP_dijagram}
...
\begin{itemize}
    ...
    \item \textbf{Učesnici:}
        ...
        \end{itemize}
    \item \textbf{Preduslovi:} Sistem je aktivan. Neprijavljena osoba je registorvana kao korisnik sistema i ima pristup internetu. Osoba je uspešno ulogovana na sistem.
    \item \textbf{Postuslovi:} Korisnik je dobio potvrdu o zakazanom terminu i mestu prve vakcinacije.
    \item \textbf{Osnovni tok:}
        \begin{enumerate}
            \item Osoba pristupa delu sistema za online prijavu.
...
\begin{itemize}
    ...
    \item \textbf{Osnovni tok:}
        \begin{enumerate}
	    \item Osoba na informacionom pultu daje potrebne informacije za registraciju, zdravstvenu knjižicu i neki lični dokument za evidenciju medicinskom radniku.
	    \item Medicinski radnik unosi podatke u sistem i potvrđuje unos.
	    \item Sistem vrši validaciju podataka.
            \item Sistem čuva podatke i prikazuje informacije o nalogu koji kreira i traži potvrdu registracije.
	    ...
	    \item Medicinski radnik od osobe traži da odabere ustanove i proizvođače vakcina i osoba se izjašnjava po tom pitanju.
	    \item Medicinski radnik unosi podatke u sistem i potvrđuje.
	    \item Sistem daje pregled slobodnih termina.
            \item Medicinski radnik obaveštava osobu o slobodnim terminima i osoba bira termin.
	    \item Medicinski radnik bira odgovarajući termin u sistemu i potvrđuje.
            \item Sistem zakazuje termin vakcinacije.
	    \item Sistem osobi šalje email sa potvrdom o zakazanom terminu i obaveštava korisnika sistema o uspešnom zakazivanju termina.
    ...
    \item \textbf{Alternativni tokovi:}
        \begin{itemize}
            \item[A1.] \textbf{Neuspešna validacija podataka.} Ukoliko u koraku 3 osnovnog toka sistem naiđe na neispravne podatke, obaveštava medicinskog radnika i zahteva ponovni unos onih polja gde su nevalidni podaci. Pri čemu označava polja sa nevalidnim podacima. Kada medicinski radnik unese sve podatke ispravno, proces se nastavlja u koraku 4 osnovnog toka.
	     \item[A2.] \textbf{Nema slobodnih termina.} Ukoliko u koraku 9 osnovnog toka medicinski radnik dobije obaveštenje od sistema da nema slobodnih termina za zadate ustanove i vakcine, on informaciju prenosi osobi i proces se nastavlja u koraku 7 osnovnog toka.
        \end{itemize}
...
\end{itemize}
 ...
 \begin{enumerate}
	...
	\end{itemize}
    \item \v{C}lan medicinskog osoblja popunjava prazno polje mejl adresom.
    \item \v{C}lan medicinskog osoblja klikom na dugme potvr\dj{}uje akciju.
    \item Sistem vr\v{s}i validaciju unete mejl adrese.
   \item Sistem sa mejl adrese prijavljenog \v{c}lana medicinskog osoblja \v{s}alje mejl sa statistikom na unetu adresu koja pripada  nadle\v{z}nom licu iz spoljnog zdravstvenog sistema.
   \item Sistem \v{c}uva podatke o datumu i vremenu prosle\dj{}ivanja podataka.
 
 \end{enumerate}
 \item \textbf{Alternativni tokovi:}
 \begin{itemize}
            \item[A1.] \textbf{Unos neispravne mejl adrese.} Ukoliko u koraku 5 osnovnog toka sistem utvrdi da uneta mejl adresa ne pripada  licu iz spoljnog zdravstvenog sistema koje je nadle\v{z}no za prihvatanje statistike, u\v{c}esnik o tome biva obave\v{s}ten i proces se nastavlja u koraku 2 osnovnog toka.
        \end{itemize}
\end{itemize}
...
\begin{itemize}
    ...
    \item \textbf{Učesnici:}
        ...
        \end{itemize}
    \item \textbf{Preduslovi:} Sistem je aktivan. Osoba ima pristup internetu i prijavljena je na sistem.
    ...
    \item \textbf{Alternativni tokovi:}
        \begin{itemize}
            \item[A1.] \textbf{Uslovi za izdavanje nisu zadovoljeni.} Ukoliko u koraku 9 osnovnog toga sistem ustanovi da nisu zadovoljeni odgovarajući uslovi za izdavanje propusnice, obaveštava osobu. Proces se završava.
        \end{itemize}
...
\begin{figure}[H]
\centering
\includegraphics[scale=0.35]{Izdavanje_propusnice}
...
\end{itemize}
 ...
 \begin{enumerate}
    ...
    \item Sistem \v{s}alje zahtev u pdf formatu sa mejl adrese prijavljenog \v{c}lana medicinskog osoblja na mejl adresu nadle\v{z}nog lica iz spoljnog zdravstvenog sistema.
 \end{enumerate}
 \item \textbf{Alternativni tokovi:}
 \begin{itemize}
            \item[A1.] \textbf{Neispravno podno\v{s}enje zahteva.} Ukoliko u koraku 4 osnovnog toka sistem utvrdi da \v{c}lan medicinskog osoblja nije popunio formular kako je predvi\dj{}eno, on o tome biva obave\v{s}ten. Proces se nastavlja u koraku 3 osnovnog toka. 
        \end{itemize}
\end{itemize}
...
\end{itemize}
 ...
 \begin{enumerate}
   ...
   \item Sistem prikazuje stranicu sa praznim poljem za unos identifikacionog broja zahteva.
    ...
    \item Sistem \v{c}lanu medicinskog osoblja prikazuje stranicu sa informacijama o podnetom zahtevu ( informacije o vremenu podnetog zahteva, zatra\v{z}enim zalihama) kao i dugme za potvrdu izvr\v{s}ene transakcije.
    ...
    \item \v{C}lan medicinskog osoblja klik\'{c}e na dugme za potvrdu.
    \item Sistem informacije o transakciji \v{c}uva i a\v{z}urira podatke vezane za preostale zalihe u kovid ambulanti.
 \end{enumerate}
 \item \textbf{Alternativni tokovi:}
 \begin{itemize}
	\item [A1.] \textbf{Nepoklapanje u broju zahtevanih i pristiglih zaliha.} Ukoliko je u koraku 7  uo\v{c}eno nepoklapanje pristiglih zaliha sa onom koja je zahtevana, o tome se obave\v{s}tava spoljni nadle\v{z}ni zdravstveni sistem, zahtev automatski biva ponovo napravljen, sa novim identifikacionim brojem, a transportno vozilo sa dostavljenom robom biva vra\'{c}eno.
        \end{itemize}
\end{itemize}
...
Klasa podataka Zahtev za dobiijanje novih zaliha je centralna klasa podataka za eksternu razmenu  sredstava. Kroz nju na\v{s} sistem realizuje funkcionalnost  regulisanja koli\v{c}ine dostupnih zaliha koje osobe mogu da koriste u kovid ambulanti  u svrhu za\v{s}tite od virusa COVID-19.
\\~\\
...
Klasa Medicinski radnik je sa klasom Zahtev za dobijanje novih zaliha  u odnosu podnosi. 

\subsection{Podaci o prosle\dj{}ivanju podataka}

Klasa podataka Prosle\dj{}ivanje podataka je centralna klasa podataka za eksternu razmenu podataka. Kroz nju na\v{s} sistem realizuje funkcionalnost  prosle\dj{}ivanja podataka spoljnom nadle\v{z}nom  zdravstvenom sistemu.
\\~\\
...
Informacioni sistem kovid centra sačinjen je od tri glavna podsistema:
\begin{enumerate}
    \item \textbf{Medicinsko osoblje} - omogućava korišćenje informacionog sistema od strane osoblja i koristi usluge sledećih komponenti:
    \begin{itemize}
        \item Regulisanje zaliha - omogućava slanje zahteva i prijem novih zaliha.
        \item Prosleđivanje statistike - omogućava prikupljanje i slanje podataka zdravstvenom sistemu.
        \item Unos podataka - omogućava evidentiranje pruženih usluga korisnicima.
        \item Autentikacija - omogućava pristup sistemu.
    \end{itemize}
    \item \textbf{Pacijent} - omogućava korišćenje informacionog sistema od strane osoba koje koriste usluge centra i koristi sledeće komponente:
	\begin{itemize}
	        \item Registracija - omogućava registrovanje novog korisnika.
        	\item Autentikacija - omogućava pristup sistemu.
        	\item Izdavanje i validacija kovid propusnice - omogućava korisniku da dobije kovid propusnicu.
    \end{itemize}
    \item \textbf{Kontrola kovid propusnice} - omogućava proveru ispravnosti kovid propusnice i koristi komponentu:
	\begin{itemize}
        	\item Izdavanje i validacija kovid propusnice - omogućava kontroloru proveru validnosti propusnice.
    \end{itemize}
\end{enumerate}

\indent Takođe, nabrojane komponente koriste komponentu Baza podataka putem odgovarajućeg interfejsa. Na Slici \ref{slk:komponente} prikazan je dijagram komponenti arhitekture sistema.

\begin{figure}[H]
\centering
\includegraphics[scale=0.4]{Dijagram_komponenti}
\caption{Dijagram komponenti - softverska arhitektura}
\label{slk:komponente}
\end{figure}

Informacioni sistem je podeljen na server baze podataka i aplikacioni server koji komuniciraju pomoću TCP/IP protokola. Na serveru baze podataka se nalazi MySQL okruženje i baza podataka. Na aplikacionom serveru se koristi JSP server Tomcat 8 Catalina Servlet 3.1 i na njemu se izvršava aplikacija kovid\_centar.war. Konfiguracioni fajl aplikacije je web.xml. Deo medicinsko\_osoblje.jar implementira komponentu Korisnik - medicinsko osoblje, deo pacijent.jar implementira komponentu Korisnik - pacijent, a deo kontrola\_kovid\_propusnice.jar implementira komponentu Kontrola kovid propusnice. Aplikacija koristi biblioteku web\_tool\_lib.jar. Na Slici \ref{slk:isporucivanje} je prikazan dijagram isporučivanja sistema.

\begin{figure}[H]
\centering
\includegraphics[scale=0.33]{Dijagram_isporucivanja}
...
\begin{figure}[H]
\centering
\includegraphics[scale=0.23]{kriterijumZaPropusnicu}
\caption{Korisni\v{c}ki interfejs - strana za odabir kriterijuma izdavanja kovid propusnice}
\end{figure}
